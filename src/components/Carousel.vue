...
<template>
  <Carousel :wrap-around="true" :items-to-scroll="1" :items-to-show="1" :autoplay="4000">
    <Slide v-for="image in images" :key="image.id">
      <RouterLink :to="image.to" style="height:100%; width: 100%;" class="relative">
        <h2
          class="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl text-white
          font-bold absolute left-0 top-1/4 pl-20 lg:pl-44 2xl:pl-72"
        >
          <span class="fontKnewave">{{ image.des }}</span>
        </h2>
        <img
          :src="image.url"
          :alt="'캐러셀 이미지' + image.id"
